...
PIDS=()
export NO_PROXY="127.0.0.1,localhost,0.0.0.0,172.17.0.2,${NO_PROXY:-}"
...
wait_for_server() {
  ...
  while true; do
    if curl --noproxy '*' --silent --output /dev/null --fail "http://127.0.0.1:${port}/health"; then
...
main() {
    ...
    # =============================================================================
    cd ../../benchmarks/
    NO_PROXY="$NO_PROXY" no_proxy="$no_proxy" vllm bench serve --port 10001 --seed "$(date +%s)" \
        --model "$MODEL" \
        --dataset-name random --random-input-len 512 --random-output-len 16 \
        --num-prompts 1 --burstiness 1 --request-rate 0.1 | tee benchmark.log
