...
class ReqMeta:
    # Request Id
    request_id: str
    # Original external request id generated before EngineCore rewrites.
...
class P2pNcclConnector(KVConnectorBase_V1):
    def __init__(
        ...
        self.chunked_prefill: dict[
            str,
            tuple[list[int], list[int] | None, str | None],
        ] = {}
        self._external_req_ids: dict[str, str] = {}
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        ...
        if attn_metadata is None:
            logger.info(
                "P2P_CONNECTOR_LOAD_SKIP rank=%d reason=no_attn_metadata",
                self._rank,
            )
            return
        ...
        logger.info(
            "P2P_CONNECTOR_LOAD_BEGIN rank=%d request_count=%d layer_count=%d "
            "attn_metadata_type=%s kwargs_keys=%s",
            self._rank,
        ...
        for request in metadata.requests:
            ...
            self._external_req_ids[request_id] = external_req_id
            ip, port = self.parse_request_id(external_req_id, False)
            remote_address = ip + ":" + str(port + self._rank)
            logger.info(
                "P2P_CONNECTOR_LOAD_REQUEST_BEGIN request_id=%s "
                "external_req_id=%s remote_address=%s rank=%d "
                "block_count=%d num_tokens=%d",
                request_id,
                external_req_id,
                remote_address,
                self._rank,
            ...
            for layer_name in forward_context.no_compile_layers:
                ...
                if kv_cache is None:
                    logger.info(
                        "P2P_CONNECTOR_LAYER_SKIP request_id=%s layer_name=%s "
                        "rank=%d reason=no_kv_cache_attr layer_type=%s",
                        request_id,
                        layer_name,
                        self._rank,
                        type(layer).__name__,
                ...
                logger.info(
                    "P2P_CONNECTOR_LAYER_RECV_BEGIN request_id=%s "
                    "external_req_id=%s layer_name=%s remote_address=%s "
                    "rank=%d target=%s",
                    request.request_id,
                    external_req_id,
                    layer_name,
                    remote_address,
                    self._rank,
                    _tensor_summary(layer),
                )

                kv_cache = self.p2p_nccl_engine.recv_tensor(
                    external_req_id + "#" + layer_name, remote_address
                )

                if kv_cache is None:
                    logger.warning(
                        "P2P_CONNECTOR_LAYER_RECV_EMPTY request_id=%s "
                        "external_req_id=%s layer_name=%s remote_address=%s "
                        "rank=%d duration_ms=%.3f",
                        request.request_id,
                        external_req_id,
                        layer_name,
                        remote_address,
                ...
                logger.info(
                    "P2P_CONNECTOR_LAYER_RECV_DONE request_id=%s "
                    "external_req_id=%s layer_name=%s remote_address=%s "
                    "rank=%d duration_ms=%.3f kv=%s",
                    request.request_id,
                    external_req_id,
                    layer_name,
                    remote_address,
                ...
                logger.info(
                    "P2P_CONNECTOR_LAYER_INJECT_DONE request_id=%s "
                    "external_req_id=%s layer_name=%s remote_address=%s "
                    "rank=%d target=%s",
                    request.request_id,
                    external_req_id,
            ...
            logger.info(
                "P2P_CONNECTOR_LOAD_REQUEST_DONE request_id=%s "
                "external_req_id=%s remote_address=%s rank=%d",
                request_id,
                external_req_id,
                remote_address,
        ...
        logger.info(
            "P2P_CONNECTOR_LOAD_END rank=%d request_count=%d",
            self._rank,
            len(metadata.requests),
        )
    ...
    def save_kv_layer(
        ...
        **kwargs: Any,
        ...
        logger.info(
            "P2P_CONNECTOR_SAVE_BEGIN rank=%d layer_name=%s request_count=%d "
        ...
        for request in connector_metadata.requests:
            ...
            external_req_id = request.external_req_id or request_id
            self._external_req_ids[request_id] = external_req_id
            ip, port = self.parse_request_id(external_req_id, True)
            remote_address = ip + ":" + str(port + self._rank)

            kv_cache = extract_kv_from_layer(kv_layer, request.block_ids)
            if kv_cache is None:
                logger.warning(
                    "P2P_CONNECTOR_SAVE_SKIP request_id=%s external_req_id=%s "
                    "layer_name=%s remote_address=%s rank=%d "
                    "reason=unsupported_layout kv_layer_shape=%s",
                    request_id,
                    external_req_id,
                    layer_name,
                    remote_address,
                ...
                continue
            logger.info(
                "P2P_CONNECTOR_SAVE_REQUEST request_id=%s external_req_id=%s "
                "layer_name=%s remote_address=%s rank=%d block_count=%d kv=%s",
                request_id,
                external_req_id,
                layer_name,
                remote_address,
            ...
            self.p2p_nccl_engine.send_tensor(
                external_req_id + "#" + layer_name, kv_cache, remote_address
            )
            logger.info(
                "P2P_CONNECTOR_SAVE_REQUEST_DONE request_id=%s "
                "external_req_id=%s layer_name=%s remote_address=%s rank=%d",
                request_id,
                external_req_id,
                layer_name,
                remote_address,
        ...
        logger.info(
            "P2P_CONNECTOR_SAVE_END rank=%d layer_name=%s request_count=%d",
            self._rank,
            layer_name,
            len(connector_metadata.requests),
        )
    ...
    def wait_for_save(self):
        if self.is_producer:
            assert self.p2p_nccl_engine is not None
            logger.info("P2P_CONNECTOR_WAIT_FOR_SAVE_BEGIN rank=%d", self._rank)
            self.p2p_nccl_engine.wait_for_sent()
            logger.info("P2P_CONNECTOR_WAIT_FOR_SAVE_END rank=%d", self._rank)
    ...
    def get_finished(
        ...
        logger.info(
            "P2P_CONNECTOR_GET_FINISHED rank=%d finished_req_count=%d ids=%s",
            self._rank,
            len(finished_req_ids),
            sorted(finished_req_ids),
        )
        translated_finished_req_ids = {
            self._external_req_ids.get(request_id, request_id)
            for request_id in finished_req_ids
        }
        result = self.p2p_nccl_engine.get_finished(
            translated_finished_req_ids,
            no_compile_layers,
        )
        for request_id in finished_req_ids:
            self._external_req_ids.pop(request_id, None)
        return result
    ...
    def build_connector_meta(
        ...
        for i, req_id in enumerate(cached_reqs.req_ids):
            ...
            if self.is_producer:
                ...
                external_req_id = self.chunked_prefill[req_id][2]
